#!/bin/sh
#SBATCH -J rheMac3.unmap     # Job Name.
...
cp $WORK/genomes/rheMac3/rheMac3.rRNA.fa.gz /tmp/rm3.fa.gz
cd /tmp

## Index the databse.
gt suffixerator -dna -pl -tis -suf -lcp -v -parts 4 -db /tmp/rm3.fa.gz -indexname $SCRATCH/reads
...
gt tallymer search -output qseqnum qpos counts sequence -strand fp -tyr $SCRATCH/30mers -q /tmp/rm3.fa.gz | gzip > $SCRATCH/30mers.gtTxt.gz &
gt tallymer search -output qseqnum qpos counts sequence -strand fp -tyr $SCRATCH/50mers -q /tmp/rm3.fa.gz | gzip > $SCRATCH/50mers.gtTxt.gz &
gt tallymer search -output qseqnum qpos counts sequence -strand fp -tyr $SCRATCH/25mers -q /tmp/rm3.fa.gz | gzip > $SCRATCH/25mers.gtTxt.gz &

wait

cp $SCRATCH/*gtTxt.gz $WORK/genomes/rheMac3/unmap/
