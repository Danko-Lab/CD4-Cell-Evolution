## train_dbn.R -- Trains an DBN to detect regulatory regions based on DNAse-1 data.
##
...
positive_pos <- lapply(rep("CD4.chromHMM.Ernst2010.hg19.Prom.bed",6), function(x) { read.table(x)})
extra_enrich_bed <- lapply(rep("GencodeMerge.IntersectOpStrand.bed",6), function(x) {read.table(x)})
allow_bed <- lapply(rep("dnase1.peaks_peaks.narrowPeak",6), function(x) {read.table(x)})
...
adbn <- regulatory_dbn(gdm, adbn, ps_plus_path, ps_minus_path, inf_positions, positive_pos, n_train=15000, n_eval=0, extra_enrich_bed= extra_enrich_bed, training_mode="pretrain")

## Refine based on H-U through backprop.
adbn <- regulatory_dbn(gdm, adbn, ps_plus_path[[1]], ps_minus_path[[1]], inf_positions[[1]], positive_pos[[1]], n_train=150000, n_eval=2000, allow= allow_bed[[1]], extra_enrich_bed= extra_enrich_bed[[1]], training_mode="refine", enrich_frac= 0.15, enrich_negative_near_pos= 0.2)
